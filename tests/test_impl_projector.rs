...
impl Default for InMemoryEventStore {
    fn default() -> Self {
        Self { store: Arc::new(RwLock::new(BTreeSet::new())) }
    }
}

impl InMemoryEventStore {
    pub async fn write(&self, id: String, seq: i64, registry_key: String, bytes: Vec<u8>) {
        let row = Row { id, seq, registry_key, bytes };
...
impl ReadProtocol for InMemoryEventStore {
    async fn read(&self, id: &EntityId, start: i64, to: i64) -> Result<BTreeSet<Payload>, ProtocolError> {
        ...
        let col = self.store.read().await.iter()
            .cloned()
