...
pub trait ReadProtocol: 'static + Sync + Send {
    async fn read(&self, id: &EntityId, start: i64, to: i64) -> Result<BTreeSet<Payload>, ProtocolError>;
...
impl Projector {
    pub fn new(provider: impl ReadProtocol) -> Self {
        Self { reader: Arc::new(provider) }
    }

    pub fn of<T>(&self, entity: Option<T>) -> Material<T> {
        Material { entity, protocol: self.clone() }
    }
}

impl<T: ResolveMapping> Material<T> {
    pub async fn projection_to_latest(mut self, id: &impl ToEntityId) -> Result<Option<(T, i64)>, ProjectionError> {
        let mut mapper = Mapper::default();
        T::mapping(&mut mapper);

        let bin = self.protocol.reader.read_to_latest(&id.to_entity_id()).await.unwrap();

        let parts = bin.into_iter()
            .map(|payload| {
                let refs = mapper.find_by_key(payload.registry_key)
                    .ok_or(ProjectionError::NotCompatible)?;
                Ok(FixtureParts { seq: payload.sequence_id, bytes: payload.bytes, refs })
            })
