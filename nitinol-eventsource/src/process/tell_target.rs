...
pub trait AggregateTellTarget<A: Aggregate>: Clone + Send + Sync + 'static {
    ...
    ///
    /// Used by higher-level consumers (e.g. `TellIntent`) to identify this
    /// target without a round-trip to the aggregate's process.
    ///
    /// Implementations **must** return the actual stream key of the target
    /// aggregate.  Returning an empty string is rejected at `TellIntent::new`
    /// construction time with a panic.
    ///
